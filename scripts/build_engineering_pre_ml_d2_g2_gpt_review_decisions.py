from __future__ import annotations

import argparse
import csv
from collections import Counter
from pathlib import Path


REVIEW_DATE = "2026-05-13"
SOURCE_SLUG = "pre_ml_d2_g2_gpt_review_decisions"

FIELDS = [
    "school_key",
    "school_name",
    "decision_register_lane",
    "gate_status",
    "readiness_band",
    "review_lane",
    "review_risk_score",
    "decision_status",
    "review_decision",
    "reviewer",
    "decision_time",
    "evidence_grade",
    "caution_class",
    "acceptance_basis",
    "blocking_issues",
    "required_row_fixes",
    "residual_followups",
    "ml_boundary_note",
    "reference_year",
    "latest_year",
    "data_completeness",
    "total_plan_count",
    "minimum_score",
    "minimum_rank",
    "trend_available",
    "trend_signal",
    "gap_signature",
    "resolution_status",
    "plan_source_resolution",
    "score_source_resolution",
    "structured_plan_rows",
    "structured_score_major_rows",
    "structured_score_summary_rows",
    "plan_source_url",
    "score_source_url",
    "required_checks",
    "pass_route",
    "fail_route",
    "record_id",
    "source_record_id",
    "source_slug",
]

SUMMARY_FIELDS = [
    "school_key",
    "school_name",
    "review_decision",
    "evidence_grade",
    "caution_class",
    "minimum_score",
    "minimum_rank",
    "total_plan_count",
    "trend_signal",
    "resolution_status",
    "required_row_fixes",
    "residual_followups",
]


def read_rows(path: Path) -> list[dict[str, str]]:
    if not path.exists():
        return []
    with path.open("r", encoding="utf-8", newline="") as file:
        return list(csv.DictReader(file))


def write_rows(path: Path, rows: list[dict[str, str]], fieldnames: list[str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)


def normalize_text(value: str) -> str:
    return (
        str(value or "")
        .replace("（", "(")
        .replace("）", ")")
        .replace("，", ",")
        .replace("＋", "+")
        .strip()
    )


def parse_int(value: str) -> int:
    text = normalize_text(value)
    if not text:
        return 0
    try:
        return int(float(text))
    except ValueError:
        return 0


def keyed(rows: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    return {normalize_text(row.get("school_key", "")): row for row in rows if row.get("school_key")}


def split_flags(value: str) -> set[str]:
    return {flag for flag in normalize_text(value).split("|") if flag}


def source_label_followup(school_name: str) -> list[str]:
    label_terms = ["招生网", "本科招生", "招生信息"]
    if any(term in school_name for term in label_terms):
        return ["public_school_name_label_cleanup_optional"]
    return []


def source_urls_need_fix(*urls: str) -> bool:
    suspicious_terms = ["yoursite.com", "example.com", "localhost"]
    joined = " ".join(normalize_text(url) for url in urls)
    return any(term in joined for term in suspicious_terms)


def derive_decision(
    decision_row: dict[str, str],
    gate_row: dict[str, str],
    workbench_row: dict[str, str],
    resolution_row: dict[str, str],
) -> tuple[str, str, str, str, list[str], list[str], list[str], list[str], list[str]]:
    basis: list[str] = []
    blockers: list[str] = []
    row_fixes: list[str] = []
    followups: list[str] = []

    flags = split_flags(workbench_row.get("review_focus_flags", ""))
    reference_year = normalize_text(gate_row.get("reference_year", ""))
    latest_year = normalize_text(gate_row.get("latest_year", ""))
    resolution_status = normalize_text(gate_row.get("resolution_status", ""))
    data_completeness = normalize_text(gate_row.get("data_completeness", ""))
    trend_available = normalize_text(gate_row.get("trend_available", ""))
    blocker_class = normalize_text(gate_row.get("blocker_class", ""))
    structured_plan_rows = parse_int(resolution_row.get("structured_plan_rows", ""))
    structured_score_rows = parse_int(resolution_row.get("structured_score_major_rows", ""))
    total_plan_count = parse_int(gate_row.get("total_plan_count", ""))
    minimum_rank = parse_int(gate_row.get("minimum_rank", ""))

    if normalize_text(decision_row.get("decision_register_lane", "")) != "D2_ready_now_caution_pending_decision":
        blockers.append("not_d2_lane")
    else:
        basis.append("D2_ready_now_caution_pending_decision")

    if normalize_text(gate_row.get("gate_status", "")) != "G2_ready_with_caution_for_review_gate":
        blockers.append("not_g2_gate")
    else:
        basis.append("G2_ready_with_caution_for_review_gate")

    if normalize_text(gate_row.get("readiness_band", "")) != "M2_comparable_ready_with_note":
        blockers.append("not_m2_readiness")
    else:
        basis.append("M2_comparable_ready_with_note")

    if blocker_class not in {"", "none"}:
        blockers.append(f"blocker:{blocker_class}")
    else:
        basis.append("no_blocker")

    if not normalize_text(decision_row.get("plan_source_url", "")):
        blockers.append("plan_source_url_missing")
    if not normalize_text(decision_row.get("score_source_url", "")):
        blockers.append("score_source_url_missing")
    if source_urls_need_fix(
        decision_row.get("plan_source_url", ""),
        decision_row.get("score_source_url", ""),
    ):
        row_fixes.append("source_url_placeholder_needs_official_domain_fix")
    if structured_score_rows <= 0:
        blockers.append("structured_score_major_rows_missing")
    if minimum_rank <= 0:
        row_fixes.append("minimum_rank_needs_confirm")

    if reference_year != latest_year:
        row_fixes.append("reference_year_not_latest")
    if "missing_score" in flags:
        row_fixes.append("score_field_needs_confirm")
    if "missing_rank" in flags:
        row_fixes.append("rank_field_needs_confirm")
    if "not_fresh_2025" in flags:
        row_fixes.append("fresh_2025_record_needed")

    if data_completeness != "plan_and_score":
        if structured_plan_rows > 0:
            row_fixes.append("total_plan_count_or_plan_linkage_needs_fill")
        else:
            row_fixes.append("plan_side_needs_structured_fill")
    elif total_plan_count <= 0:
        row_fixes.append("total_plan_count_needs_fill")
    else:
        basis.append("plan_and_score")

    if trend_available != "true":
        row_fixes.append("trend_missing_or_unverified")
    else:
        basis.append(f"trend:{normalize_text(gate_row.get('trend_signal', ''))}")

    if resolution_status == "exact_ready":
        basis.append("exact_ready")
    elif resolution_status == "mixed_ready":
        row_fixes.append("mixed_source_resolution_needs_upgrade")
    elif resolution_status == "fallback_ready":
        row_fixes.append("fallback_source_resolution_needs_upgrade")
    else:
        row_fixes.append("source_resolution_needs_classification")

    followups.extend(source_label_followup(normalize_text(decision_row.get("school_name", ""))))

    if blockers:
        return (
            "hold_before_ml",
            "completed_gpt_review_decision",
            "D_hold_due_blocker",
            "blocked_caution",
            basis,
            blockers,
            row_fixes,
            followups or ["none"],
            sorted(flags),
        )

    fallback_heavy = resolution_status == "fallback_ready" and (
        structured_plan_rows <= 0
        or trend_available != "true"
        or "not_fresh_2025" in flags
        or "missing_plan" in flags
    )
    if fallback_heavy:
        return (
            "hold_before_ml",
            "completed_gpt_review_decision",
            "D_hold_before_ml",
            "fallback_heavy_caution",
            basis,
            blockers,
            row_fixes,
            followups or ["none"],
            sorted(flags),
        )

    needs_row_fix = bool(row_fixes) and (
        reference_year != latest_year
        or "missing_score" in flags
        or "missing_rank" in flags
        or resolution_status == "mixed_ready"
        or "not_fresh_2025" in flags
        or source_urls_need_fix(
            decision_row.get("plan_source_url", ""),
            decision_row.get("score_source_url", ""),
        )
    )
    if needs_row_fix:
        return (
            "request_row_fix",
            "completed_gpt_review_decision",
            "C_row_fix_required",
            "fixable_caution",
            basis,
            blockers,
            row_fixes,
            followups or ["none"],
            sorted(flags),
        )

    return (
        "accept_for_review_gate",
        "completed_gpt_review_decision",
        "B_caution_gate_accept",
        "limited_caution",
        basis,
        blockers,
        row_fixes,
        followups or ["none"],
        sorted(flags),
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build GPT-reviewed D2/G2 pre-ML review-gate decision overlay."
    )
    parser.add_argument(
        "--decision-register",
        type=Path,
        default=Path("clean_data")
        / "engineering_guangxi_seed"
        / "guangxi_pre_ml_review_gate_decision_register_merged.csv",
    )
    parser.add_argument(
        "--gate-status",
        type=Path,
        default=Path("clean_data") / "engineering_guangxi_seed" / "guangxi_pre_ml_gate_status_merged.csv",
    )
    parser.add_argument(
        "--workbench",
        type=Path,
        default=Path("clean_data") / "engineering_guangxi_seed" / "guangxi_pre_ml_review_workbench_merged.csv",
    )
    parser.add_argument(
        "--source-resolution",
        type=Path,
        default=Path("clean_data") / "engineering_guangxi_seed" / "guangxi_source_resolution_matrix_merged.csv",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("clean_data")
        / "engineering_guangxi_seed"
        / "guangxi_pre_ml_d2_g2_gpt_review_decisions_merged.csv",
    )
    parser.add_argument(
        "--school-summary-output",
        type=Path,
        default=Path("reports") / "engineering_pre_ml_d2_g2_gpt_review_decisions_school_summary.csv",
    )
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=Path("reports") / "engineering_pre_ml_d2_g2_gpt_review_decisions_coverage_rollup.csv",
    )
    return parser


def main() -> None:
    args = build_parser().parse_args()
    decision_rows = read_rows(args.decision_register)
    gate_by_key = keyed(read_rows(args.gate_status))
    workbench_by_key = keyed(read_rows(args.workbench))
    resolution_by_key = keyed(read_rows(args.source_resolution))

    output_rows: list[dict[str, str]] = []
    decision_counts: Counter[str] = Counter()
    grade_counts: Counter[str] = Counter()
    caution_counts: Counter[str] = Counter()

    for decision_row in decision_rows:
        if normalize_text(decision_row.get("decision_register_lane", "")) != "D2_ready_now_caution_pending_decision":
            continue
        school_key = normalize_text(decision_row.get("school_key", ""))
        gate_row = gate_by_key.get(school_key, {})
        workbench_row = workbench_by_key.get(school_key, {})
        resolution_row = resolution_by_key.get(school_key, {})

        (
            review_decision,
            decision_status,
            evidence_grade,
            caution_class,
            basis,
            blockers,
            row_fixes,
            followups,
            flags,
        ) = derive_decision(decision_row, gate_row, workbench_row, resolution_row)

        decision_counts[review_decision] += 1
        grade_counts[evidence_grade] += 1
        caution_counts[caution_class] += 1

        output_rows.append(
            {
                "school_key": school_key,
                "school_name": normalize_text(decision_row.get("school_name", school_key)),
                "decision_register_lane": normalize_text(decision_row.get("decision_register_lane", "")),
                "gate_status": normalize_text(gate_row.get("gate_status", "")),
                "readiness_band": normalize_text(gate_row.get("readiness_band", "")),
                "review_lane": normalize_text(workbench_row.get("review_lane", "")),
                "review_risk_score": normalize_text(workbench_row.get("review_risk_score", "")),
                "decision_status": decision_status,
                "review_decision": review_decision,
                "reviewer": "codex_gpt_review",
                "decision_time": REVIEW_DATE,
                "evidence_grade": evidence_grade,
                "caution_class": caution_class,
                "acceptance_basis": "|".join(basis) if basis else "none",
                "blocking_issues": "|".join(blockers) if blockers else "none",
                "required_row_fixes": "|".join(row_fixes) if row_fixes else "none",
                "residual_followups": "|".join(followups),
                "ml_boundary_note": "D2/G2 决策最多进入带备注复核闸门；进入 canonical/ML 前必须先完成 required_row_fixes 或保留隔离标记",
                "reference_year": normalize_text(gate_row.get("reference_year", "")),
                "latest_year": normalize_text(gate_row.get("latest_year", "")),
                "data_completeness": normalize_text(gate_row.get("data_completeness", "")),
                "total_plan_count": normalize_text(gate_row.get("total_plan_count", "")),
                "minimum_score": normalize_text(gate_row.get("minimum_score", decision_row.get("minimum_score", ""))),
                "minimum_rank": normalize_text(gate_row.get("minimum_rank", decision_row.get("minimum_rank", ""))),
                "trend_available": normalize_text(gate_row.get("trend_available", "")),
                "trend_signal": normalize_text(gate_row.get("trend_signal", "")),
                "gap_signature": "|".join(flags) if flags else "none",
                "resolution_status": normalize_text(gate_row.get("resolution_status", "")),
                "plan_source_resolution": normalize_text(gate_row.get("plan_source_resolution", "")),
                "score_source_resolution": normalize_text(gate_row.get("score_source_resolution", "")),
                "structured_plan_rows": normalize_text(resolution_row.get("structured_plan_rows", "")),
                "structured_score_major_rows": normalize_text(resolution_row.get("structured_score_major_rows", "")),
                "structured_score_summary_rows": normalize_text(resolution_row.get("structured_score_summary_rows", "")),
                "plan_source_url": normalize_text(decision_row.get("plan_source_url", "")),
                "score_source_url": normalize_text(decision_row.get("score_source_url", "")),
                "required_checks": normalize_text(decision_row.get("required_checks", "")),
                "pass_route": normalize_text(decision_row.get("pass_route", "")),
                "fail_route": normalize_text(decision_row.get("fail_route", "")),
                "record_id": f"{school_key}-d2-g2-gpt-review-decision",
                "source_record_id": normalize_text(decision_row.get("record_id", "")),
                "source_slug": SOURCE_SLUG,
            }
        )

    output_rows.sort(key=lambda row: (row["review_decision"], row["evidence_grade"], row["school_key"]))
    write_rows(args.output, output_rows, FIELDS)
    write_rows(
        args.school_summary_output,
        [{field: row[field] for field in SUMMARY_FIELDS} for row in output_rows],
        SUMMARY_FIELDS,
    )

    coverage_rows = [
        {"metric": "d2_g2_review_decision_rows", "value": str(len(output_rows))},
        {"metric": "completed_gpt_review_decisions", "value": str(len(output_rows))},
        {"metric": "ml_boundary_still_closed", "value": "true"},
    ]
    for decision, count in sorted(decision_counts.items()):
        coverage_rows.append({"metric": f"{decision}_rows", "value": str(count)})
    for grade, count in sorted(grade_counts.items()):
        coverage_rows.append({"metric": f"{grade}_rows", "value": str(count)})
    for caution, count in sorted(caution_counts.items()):
        coverage_rows.append({"metric": f"{caution}_rows", "value": str(count)})
    write_rows(args.coverage_output, coverage_rows, ["metric", "value"])

    print(f"Wrote D2/G2 GPT review decisions for {len(output_rows)} schools to {args.output}.")


if __name__ == "__main__":
    main()
