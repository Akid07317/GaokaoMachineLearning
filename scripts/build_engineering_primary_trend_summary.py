...
def trend_label(plan_delta: int | None, score_delta: int | None, rank_delta: int | None) -> str:
    ...
    if rank_delta is not None:
        if rank_delta > 0:
            labels.append("最低位次后移")
        elif rank_delta < 0:
            labels.append("最低位次前移")
        else:
            labels.append("最低位次持平")
    return "|".join(labels) if labels else "信息不足"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build school-year trend summaries from primary Guangxi physics snapshots."
    )
    parser.add_argument(
        "--primary-source",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("clean_data") / "engineering_guangxi_seed" / "guangxi_primary_trend_summary_merged.csv",
        help="Primary trend summary CSV.",
    )
    parser.add_argument(
        "--school-summary-output",
        type=Path,
        default=Path("reports") / "engineering_primary_trend_school_summary.csv",
        help="Primary trend school summary CSV.",
    )
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=Path("reports") / "engineering_primary_trend_coverage_rollup.csv",
        help="Primary trend coverage rollup CSV.",
    )
    parser.add_argument(
        "--round-summary-output",
        type=Path,
        default=Path("reports") / "derived_primary_trend_round.csv",
...
def main() -> None:
    ...
    for row in rows:
        grouped[
            (
                ...
                normalize_text(row.get("normalized_type", "")),
    ...
    for (school_key, normalized_type, subject_type), subset in sorted(grouped.items()):
        ordered = sorted(subset, key=lambda item: (parse_int(item.get("year", "")) or 0, item.get("record_id", "")))
        if len(ordered) < 2:
            continue
        for prev, curr in zip(ordered, ordered[1:]):
            ...
            remarks = ";".join(
                part
                for part in [
                    "由广西物理类普通批主口径快照派生的逐校逐年趋势摘要",
                    f"原始年份={normalize_text(prev.get('year', ''))}->{normalize_text(curr.get('year', ''))}",
                    f"原始类型={normalized_type}",
            ...
            trend_rows.append(
                {
                    ...
                    "from_year": normalize_text(prev.get("year", "")),
                    "to_year": normalize_text(curr.get("year", "")),
                    ...
                    "remarks": remarks,
                    "record_id": f"{school_key}-primary-trend-{normalize_text(prev.get('year', ''))}-{normalize_text(curr.get('year', ''))}-{subject_type}",
                    "from_record_id": normalize_text(prev.get("record_id", "")),
                    "to_record_id": normalize_text(curr.get("record_id", "")),
                    "source_slug": "official_primary_physics_trend_summary",
                }
            )
    ...
    write_rows(
        ...
        school_summary_rows,
        ["school_key", "school_name", "primary_trend_rows"],
    )

    coverage_rows = [
        {"metric": "target_pool_schools", "value": str(TARGET_TOTAL)},
        {"metric": "primary_trend_schools", "value": str(len(school_counter))},
        {"metric": "primary_trend_coverage_ratio", "value": f"{len(school_counter) / TARGET_TOTAL:.4f}"},
        {"metric": "primary_trend_total_rows", "value": str(len(trend_rows))},
    ]
