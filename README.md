...
- 每轮至少落地 5 所学校的真实新增数据。
- 优先处理已缓存的官方 HTML、PDF、JSON，其次是公开 API，再次是静态页面。
- 403、网络不通、协议阻塞学校进入单独待办，不拖慢主线。
- 先扩大覆盖学校数，再补厚单校深度。
- 每轮结束后按固定顺序刷新总表，避免并行重刷造成口径漂移。
- 本地检查与状态核对默认单命令串行执行，不把多个短检查塞进并行批处理。
- 所有易挂住动作必须短超时、可截断；先落表，再做最小范围核对。
